...
export default function TestCaseGenerator() {
  const [scrapedAnalysis, setScrapedAnalysis] =
    useState<WebsiteAnalysis | null>(null);
  const [generatedData, setGeneratedData] =
    useState<TestCaseGenerationResponse | null>(null);
  ...
  const scrapeMutation = useMutation({
    mutationFn: async ({ url, prompt }: { url: string; prompt: string }) => {
      const response = await apiRequest("POST", "/api/scrape-website", {
        url,
        prompt,
      });
    ...
    onSuccess: (data: WebsiteAnalysis) => {
      ...
      toast({
        ...
        description: `Found ${
          data.forms?.reduce(
            (sum: number, f: any) =>
              sum + (Array.isArray(f?.fields) ? f.fields.length : 0),
            0
          ) || 0
        } fields, ${data.buttonsWithSelectors?.length || 0} buttons`,
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  const fetchPreview = async (url: string) => {
    try {
      ...
      toast({
        ...
        description:
          e?.message ||
          "Could not load live preview. Check console for details.",
        variant: "destructive",
      });
    } finally {
  ...
  const executeTestCases = async () => {
    ...
    try {
      ...
      const allSteps = generatedData.testCases.flatMap(
        (tc: (typeof generatedData.testCases)[number]) => tc.steps
      );
  ...
  const executeTestCasesOnPreview = async () => {
    if (!generatedData || !iframeRef.current) {
      toast({
        title: "No test cases to execute or preview not loaded",
        variant: "destructive",
      });
      return;
    }
        ...
        try {
          ...
          if (fillMatch) {
            ...
            if (targetInput) {
              ...
              success = true;
              selector = targetInput.name
                ? `input[name="${targetInput.name}"]`
                : targetInput.id
                ? `#${targetInput.id}`
                : targetInput.tagName.toLowerCase();
            ...
            if (targetButton) {
              ...
              selector = targetButton.id
                ? `#${targetButton.id}`
                : targetButton.tagName.toLowerCase();
  ...
  function generateAnalysisSummary(analysis: WebsiteAnalysis) {
    ...
    const submitBtn = buttons.find((b: any) => {
      ...
      return (
        ...
        (b.selectors || []).some(
          (s: any) => s.includes("submit") || s.includes("login")
        )
      );
    });
    const topFields = fields
      ...
      .map((f: any) => ({
        name:
          (f as any).friendlyName || f.name || f.placeholder || f.id || f.text,
        selectors: f.selectors,
        xpath: (f as any).xpath || (f as any).xpathCandidates?.[0],
      }));
    const topButtons = buttons
      .slice(0, 6)
      .map((b: any) => ({
        text: (b as any).friendlyName || b.text || b.name || b.id,
        selectors: b.selectors,
        xpath: (b as any).xpath || (b as any).xpathCandidates?.[0],
      }));
    const suggestedFlows: string[] = [];
    if (username && password && submitBtn) {
      suggestedFlows.push(
        `Login flow: Fill "${
          username.name || username.placeholder || username.id
        }" -> Fill "${
          password.name || password.placeholder || password.id
        }" -> Click "${submitBtn.text || submitBtn.id || "submit"}"`
      );
    }
    if (fields.length)
      suggestedFlows.push(
        `Form tests: ${fields.length} input fields detected - validate required, invalid/valid inputs`
      );
    if (buttons.length)
      suggestedFlows.push(
        `Navigation tests: ${buttons.length} clickable elements detected`
      );
    return {
      ...
      usernameField: username
        ? username.name || username.id || username.placeholder
        : null,
      passwordField: password
        ? password.name || password.id || password.placeholder
        : null,
      submitButton: submitBtn
        ? submitBtn.text || submitBtn.id || submitBtn.selectors?.[0]
        : null,
      suggestedFlows,
    };
  }
    ...
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-[98vw] mx-auto">
        <div className="mb-4 text-center">
          <h1 className="text-3xl font-bold mb-2">TestifyAI</h1>
          <p className="text-muted-foreground">
            Enter URL and instructions to scrape XPaths and generate test cases
          </p>
        </div>
          ...
          <div className="space-y-4 overflow-y-auto max-h-[calc(100vh-8rem)]">
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-xs">
                  Enter the URL and describe what you want to test
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">
                    Mention fields (username, password) and buttons (login,
                    submit) you want to test
                  </p>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={!scrapedAnalysis || generateMutation.isPending}
                    variant="secondary"
                    className="flex-1 text-xs h-9"
                  >
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-xs">
                  {scrapedAnalysis
                    ? `${
                        (scrapedAnalysis.allInteractive || []).filter(
                          (e: any) =>
                            ["input", "textarea", "select"].includes(e.tag)
                        ).length
                      } fields, ${
              ...
              <CardContent>
                {!scrapedAnalysis && (
                  <div className="text-xs text-muted-foreground text-center py-4">
                    Enter URL and prompt, then click "Scrape"
                  </div>
                )}
                {scrapedAnalysis && (
                  <div className="space-y-2 max-h-[200px] overflow-y-auto">
                    {/* If explicit forms exist show them, otherwise show derived interactive elements */}
                    {scrapedAnalysis.forms &&
                    scrapedAnalysis.forms.length > 0 ? (
                      scrapedAnalysis.forms.map(
                        (
                          form: {
                            name: string;
                            fields: Array<{
                          ...
                          <div key={fi}>
                            ...
                            {form.fields.map((field, i) => (
                              <div key={i} className="ml-3 mb-1.5 text-[11px]">
                                ...
                                <code className="text-[9px] bg-muted px-1 rounded block mt-0.5 truncate">
                                  {field.selectors?.[0] || field.xpath || "—"}
                                </code>
                              </div>
                            ))}
                          </div>
                      ...
                      <div className="space-y-1">
                        {(scrapedAnalysis.allInteractive || [])
                          .slice(0, 40)
                          .map((el: any, i: number) => (
                            <div
                              key={i}
                            ...
                            >
                              ...
                              <div className="ml-3 text-right">
                                <code className="text-[9px] bg-muted px-1 rounded truncate block max-w-[140px]">
                                  -{" "}
                                  {(el.selectors && el.selectors[0]) ||
                                    (el as any).xpath ||
                                    "—"}
                                  +{" "}
                                  {(el as any).xpath ||
                                    (el as any).xpathCandidates?.[0] ||
                                    (el.selectors && el.selectors[0]) ||
                                    "—"}
                                </code>
                              </div>
                        ...
                        {(scrapedAnalysis.allInteractive || []).length > 40 && (
                          <div className="text-[10px] text-muted-foreground">
                            Showing first 40 of{" "}
                            {(scrapedAnalysis.allInteractive || []).length}{" "}
            ...
            {analysisSummary && (
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs">
                    What the analyzer found and suggested flows
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2 text-sm">
                    <div>
                      <strong>Title:</strong> {analysisSummary.title || "—"}
                    </div>
                    <div>
                      <strong>Interactive elements:</strong>{" "}
                      {analysisSummary.interactiveCount}
                    </div>
                    <div>
                    ...
                    {analysisSummary.suggestedFlows.length > 0 && (
                      <div>
                        ...
                        <ol className="list-decimal list-inside text-xs mt-1">
                          {analysisSummary.suggestedFlows.map(
                            (s: string, idx: number) => (
                              <li key={idx}>{s}</li>
                            )
                          )}
                    ...
                    <div className="mt-2 flex gap-2">
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          console.log("Full analysis:", scrapedAnalysis)
                        }
                      >
            ...
            {generatedData && (
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="text-lg">
                    Generated Test Cases ({generatedData.summary.totalTests})
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    {generatedData.testCases.map(
                      (tc: (typeof generatedData.testCases)[number]) => (
                        <Card key={tc.id}>
                          <CardHeader className="pb-2 pt-3">
                            <div className="flex items-start justify-between">
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  setExpandedTestCase(
                                    expandedTestCase === tc.id ? null : tc.id
                                  )
                                }
                                className="h-6 w-6"
                              >
            ...
            {generatedData && (
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs">
                    Run the generated test cases on the live preview
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Button
                    ...
                    <div className="flex items-center justify-between p-2 bg-muted rounded-lg">
                      <div>
                        ...
                        <div className="text-[9px] text-muted-foreground">
                          {executionResults.summary.successful} /{" "}
                          {executionResults.summary.total} steps passed
                        </div>
                      </div>
                      <Badge
                        variant={
                          executionResults.success ? "default" : "destructive"
                        }
            ...
            <Card className="h-full flex flex-col">
              ...
              <CardContent className="flex-1 flex flex-col min-h-0 pb-3">
                    ...
                    <iframe
                      ...
                      onLoad={() => {
                        ...
                        try {
                          ...
                          console.warn(
                            "[Preview] Cannot access iframe DOM (cross-origin):",
                            e
                          );
                        }
                      }}
                ...
                <div className="mt-2 text-[10px] text-muted-foreground flex items-center justify-between">
                  ...
                  {scrapedAnalysis?.url && (
                    <span className="font-mono text-[9px] truncate max-w-[300px]">
                      {scrapedAnalysis.url}
                    </span>
                  )}
                </div>
